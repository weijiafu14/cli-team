...
export function useAutoScroll({ messages, itemCount }: UseAutoScrollOptions): UseAutoScrollReturn {
  ...
  useEffect(() => {
    ...
    // Also force scroll on initial DB load (0 -> N messages), or if new messages arrived and user hasn't scrolled up
    if (lastMessage?.position === 'right' || (prevLength === 0 && currentListLength > 0) || (isNewMessage && !userScrolledRef.current)) {
      userScrolledRef.current = false;
